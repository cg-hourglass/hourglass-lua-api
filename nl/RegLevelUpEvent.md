...
## NL.RegLevelUpEvent(Dofile, FuncName)
...
## LevelUpCallBack(CharIndex)

### 参数说明

- CharIndex: [数值型](../appendix/数值型.md) 升级的玩家对象index，由引擎传入。
...
```lua
NL.RegLevelUpEvent(nil, "MyLevelUpEvent");

function MyLevelUpEvent(CharIndex)
  print("玩家"..Char.GetData(CharIndex, %对象_名字%).."升级啦！");
  return 0;
end
```
...
只在判定确实升了至少一级时触发，并且只对玩家角色触发；宠物升级走 RegPetLevelUpEvent。
