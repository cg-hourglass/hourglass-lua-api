...
- Type: [数值型](../appendix/数值型.md) 离开方式。1 = 逃跑；2 = 被必杀技踢出战斗。由引擎传入。
...
```lua
NL.RegBattleExitEvent(nil, "MyBattleExitEvent");

function MyBattleExitEvent(CharIndex, BattleIndex, Type)
  if(Type == 2)then
...
只对玩家角色触发，且只有逃跑(1)和被必杀技踢出(2)两种退出方式会进入 Lua；
正常结束战斗、登出等其它退出方式永远不会触发本事件。
